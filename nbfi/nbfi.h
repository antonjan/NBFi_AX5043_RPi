...
struct nbfi_func_t
{
	...
	void (*before_tx)(NBFi_ax5043_pins_s *);
	void (*before_rx)(NBFi_ax5043_pins_s *);
	void (*before_off)(NBFi_ax5043_pins_s *);
	void (*read_default_settings)(nbfi_settings_t*);
	void (*read_flash_settings)(nbfi_settings_t*);
	void (*write_flash_settings)(nbfi_settings_t*);
	uint32_t (*measure_voltage_or_temperature)(uint8_t);
	uint32_t (*update_rtc)(void);
	void (*rtc_synchronized)(uint32_t);
	void (*lock_unlock_nbfi_irq)(uint8_t);
	void (*reset)(void);
};
