...
struct nbfi_func_t _nbfi_func;

void NBFI_reg_func(struct nbfi_func_t *fn)
{
	memcpy(&_nbfi_func, fn, sizeof (struct nbfi_func_t));
}
...
nbfi_status_t NBFi_Send(uint8_t* payload, uint8_t length)
{
	...
	if(_nbfi_func.lock_unlock_nbfi_irq) _nbfi_func.lock_unlock_nbfi_irq(1);
    
    uint8_t free = NBFI_TX_PKTBUF_SIZE - NBFi_Packets_To_Send();
    if((length <= nbfi.max_payload_len) && (free < nbfi.mack_mode + 3 ) ) 
    {
	  if(_nbfi_func.lock_unlock_nbfi_irq) _nbfi_func.lock_unlock_nbfi_irq(0);
...
void NBFi_ProcessRxPackets(_Bool external)
{
    ...
    while(1)
    {
		...
		if(_nbfi_func.rx_handler) _nbfi_func.rx_handler(data_ptr, total_length);
...
void NBFi_ParseReceivedPacket(struct axradio_status *st)
{
    ...
    if(phy_pkt->SYS)
    {
            ...
            case 0x07: //software reset
				if(_nbfi_func.reset && (phy_pkt->payload[1] == 0xDE) && (phy_pkt->payload[2] == 0xAD)) _nbfi_func.reset();
                break;
...
static void NBFi_ProcessTasks(struct wtimer_desc *desc)
{
   ...
   if((rf_busy == 0)&&(transmit == 0)&&!((need_to_calc_noise == 1)&&(rf_state == STATE_RX)))
   {
    ...
    {
		  uint32_t t = _nbfi_func.measure_voltage_or_temperature(1);
...
static void NBFi_update_RTC()
{
	...
	if(_nbfi_func.update_rtc)
    {
	  nbfi_rtc = _nbfi_func.update_rtc();
      return;
    }
    
    uint32_t delta;
    
	uint32_t tmp = (wtimer_state[0].time >> 10);
...
void NBFi_set_RTC(uint32_t time)
{
   NBFi_update_RTC();
   nbfi_rtc = time;
   if(_nbfi_func.rtc_synchronized) _nbfi_func.rtc_synchronized(nbfi_rtc);
}
...
static void NBFi_SendHeartBeats(struct wtimer_desc *desc)
{
    ...
    if(++hb_timer >= nbfi.heartbeat_interval + 1)
    {
		...
		ack_pkt->phy_data.payload[3] = _nbfi_func.measure_voltage_or_temperature(0);    //temperature
...
nbfi_status_t NBFI_Init()
{
    ...
    else
    {
      ...
      NBFi_Force_process();
	  _nbfi_func.measure_voltage_or_temperature(1);
