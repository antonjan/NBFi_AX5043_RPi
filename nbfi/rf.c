...
void RF_SetModeAndPower(int8_t dBm, rf_direction_t mode, rf_antenna_t ant)
{
    ...
    case TX:    
	  ...
	  if(_nbfi_func.before_tx) _nbfi_func.before_tx(&nbfi_ax5043_pins);
      break;
    case RX: 
      nbfi_ax5043_pins.cfga = PA_DIFFERENTIAL;
	  if(_nbfi_func.before_rx) _nbfi_func.before_rx(&nbfi_ax5043_pins);
      break;
    case IDLE:
	  if(_nbfi_func.before_off)   _nbfi_func.before_off(&nbfi_ax5043_pins);
      break;
    }
