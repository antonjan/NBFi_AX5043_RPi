...
extern struct nbfi_func_t _nbfi_func;
...
void NBFi_ReadConfig(nbfi_settings_t *settings)
{
	...
	if(_nbfi_func.read_flash_settings == 0) goto read_default;

	_nbfi_func.read_flash_settings(settings);

	if((settings->tx_phy_channel != 0xff) && (settings->tx_phy_channel != 0)) return;

read_default:

	if(_nbfi_func.read_default_settings) _nbfi_func.read_default_settings(settings);

}

void NBFi_WriteConfig()
{
	if(_nbfi_func.write_flash_settings == 0) return;
	_nbfi_func.write_flash_settings(&nbfi);
}
...
void NBFi_Clear_Saved_Configuration()
{
  ...
  _nbfi_func.write_flash_settings(&empty);
}
